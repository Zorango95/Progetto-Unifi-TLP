...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item
			E' possibile che il tipo di una lambda sia compatibile con più target type .  \\\
\begin{lstlisting}[language=Java]
...
\begin{frame}[fragile]
	...
	\begin{itemize}
		\item
			Finora abbiamo visto alcune semplici applicazioni delle Lambda\\\
		\item
			Adesso vediamo come viene semplificato il codice nel caso dell'utilizzo dei Design Pattern.
		\item
				Design Pattern:
		\begin{itemize}
			\item
...
\begin{frame}
	\frametitle{\textbf{4.1 : Strategy}}
	\begin{figure}
		\centering
		\includegraphics[width=0.6\linewidth]{image/strategy.png}
		\label{fig:target}
	\end{figure}
	Prima dell'introduzione delle Lambda, un client che avesse voluto implementare uno Strategy avrebbe avuto due opzioni:
	\begin{itemize}
		\item
		Classe anonima;
		\item
...
\begin{frame}[fragile]
	\frametitle{\textbf{4.1 : Strategy}}
\begin{lstlisting}[language=Java]
Vendable v = new Vendable(0.5);
v.setStrategy(new Strategy(){
@Override
public double getPrice(double basePrice){
return 0.75*basePrice;		
}
});
	\end{lstlisting}
	Mentre con le Lambda è possibile fare:
\begin{lstlisting}[language=Java]
Vendable v = new Vendable(0.5);
